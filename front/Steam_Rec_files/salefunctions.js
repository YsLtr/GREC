...
function fnRenderHeroCapsule( oItem )
{
	...
	var $Cap = $J( '<div/>', {'class': 'hero_capsule', 'data-ds-appid': appids, 'data-panel': '{"clickOnActivate":"firstChild","onOptionsActionDescription":"\u6dfb\u52a0\u81f3\u8d2d\u7269\u8f66","onOptionsButton":"%onOptionsButton%","flow-children":"column"}'.replace( '%onOptionsButton%', purchaseAction ) } );
	...
	{
		$J('<div/>', {'class': 'broadcast_live_stream_icon', 'style': 'z-index:unset' } ).append( '直播')
	}
	...
	AddToCartCtn.append( $J( '<button/>', {'class': 'btn_green_steamui btn_medium', } ).click( function( e ){ eval( purchaseAction ); } ).append( $J( '<span/>' ).text( '添加至购物车' )  ) );
...
function HomeSaleSteamAwardWinners( $Parent, rgSteamAwardWinners, rgSteamAwardDefs )
{
	...
	rgSteamAwardDefs.forEach( function ( def, i )  {
		...
		let $Cap = $J( '<div/>', {'class': 'winner_ctn', 'data-panel': '{"clickOnActivate":"firstChild","onOptionsActionDescription":"\u6dfb\u52a0\u81f3\u8d2d\u7269\u8f66","onOptionsButton":"%onOptionsButton%","flow-children":"column"}'.replace( '%onOptionsButton%', purchaseAction ) } );
...
function SaleTagBlock( $Parent, rgPersonalizedTagData )
{
	...
	if(strTagMethod === "tags") { title='类似<b>游戏</b><br/>';}
	else if(strTagMethod === "gems") { title='类似<b>遗珠</b><br/>';}
	else if(strTagMethod === "default") { title='类似<b>热门游戏</b><br/>';}
	else { noTags = true;}
	...
	if ( $FeatureName === "sale_tag_bucket" )
	    $Ctn.append( $J('<a/>', {'class': 'see_more_link btnv6_white_transparent btn_small_tall', 'href': rgTagData.url } ).html( '<span>' + '查看更多' + '</span>' ) );
	else if ( $FeatureName == "sale_recommended_by_steam_labs" )
		$Ctn.append( $J('<a/>', {'class': 'see_more_link btnv6_white_transparent btn_small_tall', 'href': rgTagData.url } ).html( '<span>' + '通过交互式推荐模型发现更多' + '</span>' ) );
...
function BuildFranchiseCap( FranchiseData, bAlternate )
{
	...
	if ( FranchiseData && !FranchiseData[field] )
	{
		...
		$Cap.append( $J('<div/>', {'class': 'franchise_discount_tag' } ).text( '最多立省 {PCT}%'.replace( /\{PCT\}/, FranchiseData.nDiscountMax ) ) );
		...
		if ( !bAlternate )
		{
			...
			$Cap.one( 'mouseenter vgp_onfocus', function( event ) {
				...
				$Games.append( $J('<a/>', {'class': 'franchise_see_all', 'href': url } ).text(
					'查看其他 %s 款游戏…'.replace( /%s/, FranchiseData.nItemsTotal - FranchiseData.rgItemsToFeature.length )
				) );

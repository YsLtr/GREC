...
GHomepage = {
				...
				$el.on('hover focus', function(){
					...
					else
					{
						...
						if( rgData.tags )
						{
							...
							{
								var url = GStoreItemData.AddNavEventParamsToURL( 'https://store.steampowered.com/tags/zh-cn/TAGNAME/'.replace( /TAGNAME/, encodeURIComponent( rgData.tags[i] ) ), 'tab_preview' );
	...
	RenderRecommendedTags: function()
	{
		...
		for ( var i = 0; i < GDynamicStore.s_rgRecommendedTags.length; i++ )
		{
			...
			{
				var url = GStoreItemData.AddNavEventParamsToURL( 'https://store.steampowered.com/tags/zh-cn/TAGNAME/'.replace( /TAGNAME/, encodeURIComponent( rgTag.name ) ), 'gutter' );
...
CHomeSettings.prototype.DisplayPopup = function( $Btn )
{
	...
	if ( this.m_ApplicableSettings.new_on_steam )
		this.m_$Popup.append( this.RenderCheckbox( 'new_on_steam', 'Steam 新品' ) );
